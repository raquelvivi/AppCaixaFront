...
<template>
  ...
  <header>
    ...
    <!-- Icone de Dasbord -->
    <!-- <a href="#Dasbord"> -->
    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" @click="Dasbord" stroke-width="1.5" stroke="currentColor"
      class="size-6 ico">
  ...
  <ul class="UlLista">
    <li v-for="(item, index) in listaObjetos" :key="index" class="lista">
...
export default {
  ...
  data() {
    return {
      ...
      tdPro: 0,
      lista: [],
      listaObjetos: [{ nome: "Item", valor: "Preço Total", quantidadeV: "Quantidade" }],
  ...
  methods: {
    ...
    escolheuProdut(item){
      ...
      //lista de itens
      this.listaObjetos.push( { codigo: item.codigo, nome: item.nome, valor: (item.precovenda * this.quant), quantidade: item.quant, quantidadeV: this.quant });
      this.valorTotal = this.valorTotal + (item.precovenda * this.quant); //valor total
      this.tdPro = parseInt(this.tdPro) + parseInt(this.quant) ; // quantidade de produtos
